...
class GocodeGofmtCommand(sublime_plugin.TextCommand):
	def run(self, edit):
		view = self.view
		go_fmt_path, env = golangconfig.executable_path('gofmt', view=view)
		if go_fmt_path is None:
			go_fmt_path = 'gofmt'
		src = view.substr(sublime.Region(0, view.size()))
		gofmt = subprocess.Popen([go_fmt_path],
...
class Gocode(sublime_plugin.EventListener):
	def on_query_completions(self, view, prefix, locations):
		...
		cloc = "c{0}".format(loc)
		go_code_path, env = golangconfig.executable_path('gocode', view=view)
		if go_code_path is None:
			go_code_path = 'gocode'
		gocode = subprocess.Popen([go_code_path, "-f=csv", "autocomplete", filename, cloc],
